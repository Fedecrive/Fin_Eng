function [datesCDS, survProbs, intensities] = bootstrapCDS_exact(datesDF, discounts, datesCDS, spreadsCDS, recovery)
    % This function performs an exact bootstrap of CDS spreads 
    ...
    survProbs(2) = (1 - recovery - spreadsCDS(1) * deltas(1) / 2) / (1 - recovery + spreadsCDS(1) * deltas(1) / 2);
    ...
    for i = 2:length(survProbs)-1
        ...
        sum2 = 0;
        % Compute the second summation term with adjustment factor deltas(j)/2
        for j = 1:i-1
            sum2 = sum2 + discountsCDS(j) * (survProbs(j) - survProbs(j+1)) * deltas(j) / 2;
        end
        ...
        N = (1 - recovery) * sum1 ...
            ...
            - spreadsCDS(i) * deltas(i) * discountsCDS(i) / 2 * survProbs(i) ...
            - spreadsCDS(i) * sum2;

        D = (1 - recovery) * discountsCDS(i) ...
            + spreadsCDS(i) * deltas(i) * discountsCDS(i) ...
            - spreadsCDS(i) * deltas(i) * discountsCDS(i) / 2;
